...
public interface CirclePostMapper extends BaseMapper<CirclePost> {
     ...
     * @return 圈子帖子VO分页
     */
    @Select("select c.id as contentId,content,images,comment_count,store_logo,store_name from li_circle_post c left join li_member m on m.id=c.user_id left join li_store s on s.member_id=m.id ${ew.customSqlSegment}")
