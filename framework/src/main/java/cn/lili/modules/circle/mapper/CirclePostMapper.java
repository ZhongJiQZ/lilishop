...
public interface CirclePostMapper extends BaseMapper<CirclePost> {
     ...
     * @return 圈子帖子VO分页
     */
    @Select("select c.store_id,c.id as contentId,content,images,comment_count,IFNULL(store_logo, u.avatar) store_logo,IFNULL(store_name, u.nick_name) store_name,user_type,c.create_time from li_circle_post c left join li_member m on m.id=c.user_id left join li_store s on s.member_id=m.id LEFT JOIN li_admin_user u ON u.id = c.user_id ${ew.customSqlSegment}")
